...
local ServerListingScreen = Class(Screen, function(self, prev_screen, filters, cb, offlineMode, session_mapping, forced_settings, event_id)
    ...
    if self.forced_settings.online then
        self.default_focus = self:CurrentCenterFocus()
    else
...
function ServerListingScreen:ResetFilters()
    for i, v in ipairs(self.filters) do
        if v.spinner ~= nil and not v.is_forced then 
...
function ServerListingScreen:MakeFiltersPanel(filter_data)
    ...
    self.filters_scroll_list = self.server_detail_panel:AddChild(ScrollableList(editable_filters, scroll_width, scroll_height, item_height, item_padding, nil, nil, 0))
...
function ServerListingScreen:MakeMenuButtons(left_col, right_col, nav_col)
    ...
    if not self.forced_settings.online then
